...
def character_locator(char_x, char_y):
    ...
    elif char_x == 3 and char_y == 1: doorways = '' 
...
def mover(char_nextmove, opendoors, char_x, char_y):
    ...
    if (char_nextmove == 'n') and ('n' in opendoors):
        charmoves_y += 1; print("Current position: ", charmoves_x, charmoves_y); return charmoves_y
    elif (char_nextmove == 's') and ('s' in opendoors):
        charmoves_y -= 1; return charmoves_y
    elif (char_nextmove == 'e') and ('e' in opendoors):
        charmoves_x += 1; return charmoves_x
    elif (char_nextmove == 'w') and ('w' in opendoors):
        charmoves_x -= 1; return charmoves_x
    else:
...
while 1:
    ...
    char_doorways = character_locator(char_x, char_y)  # character_locator(char_x, char_y) #Character locator finnur hvar character er
    ...
    char_nextmove = set_new_position() #Tekur inn input notanda um átt
    
    mover(char_nextmove, char_doorways, char_x, char_y))
